...
export function generateAnalysisPDF(analysisData: any, docTypeLabel: string) {
  ...
  const addHeader = (title: string, subtitle: string) => {
    ...
    doc.text("Namma NyayaMitra (न्यायमित्र)", margin, 18);
  ...
  const addFooter = (d: jsPDF, w: number, h: number) => {
    ...
    d.text("Disclaimer: Namma NyayaMitra provides AI-powered information, not formal legal advice.", margin, h - 17);
  ...
  // Save analysis file
  doc.save(`Namma NyayaMitra-Analysis-${docTypeLabel.replace(/\s+/g, "_")}-${new Date().toLocaleDateString("en-IN")}.pdf`);
}
...
export function generateTemplatePDF(draftData: any, typeLabel: string) {
  ...
  const addFooter = (d: jsPDF, w: number, h: number) => {
    ...
    d.setTextColor(139, 148, 158);
    d.line(margin, h - 22, w - margin, h - 22);
    d.text("Warning: This template was generated by Namma NyayaMitra AI for general business reference. Make sure to authenticate signatures.", margin, h - 17);
  ...
  doc.save(`Namma NyayaMitra-${typeLabel.replace(/\s+/g, "_")}-${new Date().toLocaleDateString("en-IN")}.pdf`);
}
